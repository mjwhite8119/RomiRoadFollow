...
def launchTensorBoard(logPath):
        import os
        os.system('tensorboard --logdir=' + logPath)
        return

def startTensorBoard(logPath): 
    ...
    t = threading.Thread(target=launchTensorBoard, args=([logPath]))
    t.start()
    ...
    for i in range(data.shape[0]):
        ...
        if data.shape[3] == 1:
            ...
            cv2.putText(img, f"{label}", 
                    (2, img.shape[0] - 4), 
                    cv2.FONT_HERSHEY_TRIPLEX, 1.0, color)
            img = np.clip(img, 0, 1)
            plt.imshow(img)
